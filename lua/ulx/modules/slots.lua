...
function calcSlots(disconnect)
    local mode = GetConVar("ulx_rslotsMode")
    if mode == 3 then return 1 end -- Only one slot on this mode
    local slots = GetConVar("ulx_rslots")
    ...
    if mode == 1 then
        ...
        -- Otherwise we're counting the disconnecting admin
        if disconnect then
            admins = admins - 1
        end
        ...
        -- If we have more admins right now then slots for them, we don't want to return a negative number.
        if rslots < 0 then
            rslots = 0
        end
    ...
    -- We're actually having an error if we get here, but let's handle it gracefully

    return 0
end

local function updateSlots(ply, disconnect)
    local visible = ULib.toBool(GetConVar("ulx_rslotsVisible"))

    -- Make sure our visible slots is up to date
    if not visible then
...
hook.Add("PlayerDisconnected", "ulxSlotsDisconnect", function(ply)
    updateSlots(ply, ply:query(access))
end)
...
local function playerAccess(ply)
    ...
    -- If they have access, handle this differently
    if ply:query(access) then
        ...
        if mode == 3 and cur + slots > max then
            ...
            if not shortestply then return end -- We've got a server filled to the brim with admins? Odd but okay
