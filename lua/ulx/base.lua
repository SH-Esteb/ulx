...
function ulx.addToHelpManually(category, cmd, string, access_tag)
    ...
    table.insert(ulx.cmdsByCategory[category], {
...
local function sendAutocompletes(ply)
    -- Only send if they have access to this.
...
function cvarChanged(sv_cvar, cl_cvar, ply, old_value, new_value)
    ...
    if new_value:find("[%s:']") then
        new_value = string.format("%q", new_value)
    end
    ...
    -- Configuration option does not exist in config- append it
    if found == 0 then
...
-- We're not interested in changing cvars till after load
hook.Add(ulx.HOOK_ULXDONELOADING, "AddCvarHook", function()
    hook.Add(ULib.HOOK_REPCVARCHANGED, "ULXCheckCvar", cvarChanged)
end)
